...
def find_lowest_location_number(lines) -> int:
    ...
    for line in lines:
        ...
        if line.startswith('seeds: '):
            seed_ranges = [int(x.strip()) for x in line.split(':')[1].split(' ') if len(x.strip()) > 0]
        elif ':' in line:
            ...
            maps[last_source][last_dest] = sorted(m, key=lambda x: x[0])
    min_location = None
    for i in range(0, len(seed_ranges), 2):
        print('Progress seed ranges', i, '/', len(seed_ranges) / 2)
        for j in range(0, seed_ranges[i + 1]):
            seed = seed_ranges[i] + j
